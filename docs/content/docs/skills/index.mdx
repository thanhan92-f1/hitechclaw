...
HiTechClaw comes with **11 pre-built AI agent skill packs** covering diverse industry domains. Each skill pack provides a set of specialized tools that the AI agent can use.
...
Each skill is defined using the `defineSkill()` helper from `@hitechclaw-ai/core`:
