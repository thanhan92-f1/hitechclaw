...
When a user sends a **photo**, xClaw automatically:
...
For messages about time-sensitive topics (weather, news, prices, scores), xClaw automatically prepends a routing instruction telling the agent to use web-search tools first. This works for both Vietnamese and English.
