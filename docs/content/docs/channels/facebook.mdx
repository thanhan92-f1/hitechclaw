...
description: Connect xClaw to Facebook Messenger via the Meta Graph API webhook.
---
...
xClaw exposes a universal channel webhook at:
...
3. Set **Verify Token** to the same value you enter in xClaw
4. Subscribe to the **messages** and **messaging_postbacks** fields

> **Tip**: Get `<connectionId>` from the channel card in xClaw after saving the configuration.

### 3. Configure in xClaw

1. Navigate to **Channels** → **Add Channel** → **Facebook Messenger**
2. Fill in the fields below
3. Click **Test Connection** — xClaw calls the Graph API and returns your page name
4. Click **Activate**
...
- **Webhook verification**: Meta sends a `GET` with `hub.challenge` — xClaw verifies the token and echoes the challenge
