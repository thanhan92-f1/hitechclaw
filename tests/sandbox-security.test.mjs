...
describe('Policy Enforcement', async () => {
    const { PolicyBuilder, BUILTIN_POLICIES } = await import('@hitechclaw/sandbox');
...
describe('Credential Security', async () => {
    it('encrypted credentials should not contain plaintext', async () => {
        ...
        const { encryptCredentials } = await import('@hitechclaw/db');
...
describe('Sandbox Pool Security', async () => {
    const { SandboxManager } = await import('@hitechclaw/sandbox');
...
describe('OCSF Event Integrity', async () => {
    const { toOCSFEvent } = await import('@hitechclaw/sandbox');
