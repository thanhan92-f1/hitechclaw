#!/usr/bin/env bash
# ============================================================
# PostgreSQL Backup for HiTechClaw
...
FILENAME="hitechclaw_pg_${TIMESTAMP}.dump"

DB_HOST="${PG_HOST:-localhost}"
DB_PORT="${PG_PORT:-5432}"
DB_USER="${PG_USER:-hitechclaw}"
DB_NAME="${PG_DB:-hitechclaw}"

mkdir -p "$BACKUP_DIR"

echo "→ Backing up PostgreSQL ($DB_HOST:$DB_PORT/$DB_NAME)..."
PGPASSWORD="${PG_PASSWORD:-hitechclaw}" pg_dump \
  -h "$DB_HOST" \
  -p "$DB_PORT" \
  -U "$DB_USER" \
  -d "$DB_NAME" \
...
find "$BACKUP_DIR" -name "hitechclaw_pg_*.dump" -mtime +"$RETENTION_DAYS" -delete 2>/dev/null || true
